...
conda activate phi4
cd /home/swatson/work/MachineLearning/kaggle/DeepPastChallengeTranslateAkkadianEnglish
nohup python src/preprocessing.py > log/preprocessing_$(date +%Y%m%d_%H%M%S).log 2>&1 &
```

#### Setup and run model training:
```bash
conda activate phi4
cd /home/swatson/work/MachineLearning/kaggle/DeepPastChallengeTranslateAkkadianEnglish
nohup python src/training.py --config configs/model_seq2seq.yaml > log/training_seq2seq_$(date +%Y%m%d_%H%M%S).log 2>&1 &
```
...
conda activate phi4
cd /home/swatson/work/MachineLearning/kaggle/DeepPastChallengeTranslateAkkadianEnglish
nohup python src/inference.py > log/inference_$(date +%Y%m%d_%H%M%S).log 2>&1 &
```
